  ...
  run: (client, message) => list.findById(message.guild.id, async (err, doc) => {
    ...
    if (err) {
      return message.channel.send({
        embed: {
          color: '#c3abe6',
          description: (`\`<:fail:845125155008937994> [DATABASE_ERR]:\` the database responded with error: ${err.name}`)
        }
      }); //(`\`<:fail:845125155008937994> [DATABASE_ERR]:\` The database responded with error: ${err.name}`);
    } else if (!doc) {
    ...
    if (!anischedch) {
      return message.channel.send({
        embed: {
          color: '#c3abe6',
          description: (`<:fail:845125155008937994> **${message.member.displayName}**, this server's anischedule feature has been disabled.`)
        }
      }); //(`<:fail:845125155008937994> **${message.member.displayName}**, This server's anischedule feature has been disabled.`);
      ...
      const pages = new Page(descriptions.map((d, i) => {
        return new MessageEmbed()
          ...
          .addField('tips', [
            `- use [\`${client.prefix}watch\`](https://google.com) to add subscription`,
            `- use [\`${client.prefix}unwatch\`](https://google.com) to remove subscription`,
      ...
      const terminate = client.emojis.cache.get('767062250279927818') || '<:fail:845125155008937994>';
